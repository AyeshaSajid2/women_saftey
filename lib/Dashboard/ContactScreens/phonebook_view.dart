...
import 'package:women_saftey/Dashboard/Dashboard.dart';
import 'package:women_saftey/animations/bottomAnimation.dart';
  ...
  addToContacts(Contact con) {
    ...
    for (Contact c in _userSelectedContacts) {
      ...
      if (c.displayName != null && c.displayName == con.displayName) {
        alreadyInList = true;
        break;
      } else if (c.phones!.isNotEmpty && con.phones!.isNotEmpty) {
        if (c.phones!.first.value == con.phones!.first.value) {
          alreadyInList = true;
          break;
        }
      }
    ...
    if (!alreadyInList) {
      ...
      Fluttertoast.showToast(
        msg: "Already in your selected List",
      );
    }
  }
...
class ItemsTile extends StatefulWidget {
  final Function(Contact) addToContacts;
  final Contact c;
